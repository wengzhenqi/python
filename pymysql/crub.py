from dbconnn import Departents, Employees,Salary,Session

session = Session()
#######################################################
# hr = Departents(dep_id = 1, dep_name='人事部')
# session.add(hr)             #添加
######################添加字段#########################
# ops = Departents(dep_id = 2,dep_name = '运维部')
# dev = Departents(dep_id = 3, dep_name = '开发部')
# qa = Departents(dep_id = 4, dep_name = '测试部')
# finance = Departents(dep_id = 5, dep_name = '财务部')
# ui = Departents(dep_id = 6, dep_name = '设计部')
...
#     dep_id=3
# )
# session.add_all([yc, zyp, lzj, cyj, hqp, zds])
###############################查询###################
query1 = session.query(Departents)  #将class作为参数,返回实例
# # print(query1)         #query只是个sql语句
# deps = query1.all()     #取出查询的全部结果
# # print(deps)           #deps是由数据库表每行记录生成的实例 构成的列表
# for dep in query1:
